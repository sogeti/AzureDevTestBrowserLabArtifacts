...
&start cmd.exe "/k $agentExePath /configure /RunningAsService /login:$VSOUser,$VSOPass;AuthType=PAT /serverUrl:$serverUrl ""/WindowsServiceLogonAccount:NT AUTHORITY\NetworkService"" /WindowsServiceLogonPassword /WindowsServiceDisplayName:VsoBuildAgent /name:$AgentName /PoolName:$PoolName /WorkFolder:$WorkFolder /StartMode:Automatic /force /NoPrompt &exit"
# Restore original current directory.

#Start cmd.exe "/k $agentExePath /configure /login:$VSOUser,$VSOPass;AuthType=PAT /NoPrompt /ServerUrl:$serverUrl /Name:$AgentName /PoolName:$PoolName /WorkFolder:$WorkFolder /Force /RunningAsService:N &exit"
